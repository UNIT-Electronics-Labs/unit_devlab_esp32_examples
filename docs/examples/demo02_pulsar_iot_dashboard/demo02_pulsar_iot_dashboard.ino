/*
  ...
  - Lectura de sensor de temperatura analógico en A1
  ...
  - microSD CS: D5
  - Touch capacitivo: D6
  - BMI270: dirección I2C 0x69
  - Sensor de temperatura analógico: A1
*/
...
constexpr unsigned long TEMPERATURE_INTERVAL_MS = 500;
...
.temp-main{display:flex;align-items:center;gap:18px;margin-top:14px}
.temp-icon{font-size:3rem}
.temp-value{font-size:2.35rem;font-weight:800}
.temp-bar{height:12px;margin-top:16px;background:#071221;border:1px solid var(--border);border-radius:999px;overflow:hidden}
.temp-bar-fill{width:0%;height:100%;background:linear-gradient(90deg,#1e88ff,#31d18b,#ffba49,#ff5d73);transition:width .35s ease}
.temp-details{display:grid;grid-template-columns:repeat(2,1fr);gap:10px;margin-top:14px}
.temp-chart{width:100%;height:100px;margin-top:14px;background:#071221;border:1px solid var(--border);border-radius:12px}
@media(max-width:850px){
  .touch-card,.imu-card,.temp-card,.neo-card,.status-card{grid-column:1/-1}
...
<header>
  ...
  <div class="subtitle">Touch, temperatura, BMI270, microSD, NeoPixels y PWM desde una sola interfaz local.</div>
  <div class="badges">
...
<section class="card temp-card">
  <h2>Sensor de temperatura · A1</h2>
  <div class="status-line">
    <span id="tempLight" class="status-light ok"></span>
    <span id="tempStatus">Leyendo entrada analógica…</span>
  </div>
  <div class="temp-main">
    <div class="temp-icon">🌡️</div>
    <div>
      <div id="tempValue" class="temp-value">0.00 °C</div>
      <div id="tempLevel" class="label">Sin lectura</div>
    </div>
  </div>
  <div class="temp-bar"><div id="tempBarFill" class="temp-bar-fill"></div></div>
  <div class="temp-details">
    <div class="metric"><span class="label">ADC RAW</span><b id="tempRaw">0</b></div>
    <div class="metric"><span class="label">Voltaje · mV</span><b id="tempMv">0</b></div>
  </div>
  <canvas id="tempChart" class="temp-chart" width="500" height="100"></canvas>
</section>
...
async function updateState(){
  try{
    ...
    $('imuStatus').textContent=data.imu?'BMI270 conectado en 0x69':'BMI270 no disponible';
    $('imuLight').classList.toggle('ok',data.imu);

    updateTemperature(data.tempC, data.tempRaw, data.tempMv);
...
const maxTemperatureSamples=60;

function updateTemperature(tempC, raw, mv){
  $('tempValue').textContent=tempC.toFixed(2)+' °C';
  $('tempRaw').textContent=raw;
  $('tempMv').textContent=mv;
  let level='Temperatura normal';
  if(tempC<18) level='Temperatura baja';
  else if(tempC<30) level='Temperatura normal';
  else if(tempC<40) level='Temperatura elevada';
  else level='Temperatura alta';
  $('tempLevel').textContent=level;
  $('tempStatus').textContent='Sensor analógico activo';
  const percentage=Math.max(0,Math.min(100,(tempC/60)*100));
  $('tempBarFill').style.width=percentage+'%';
  temperatureHistory.push(tempC);
  if(temperatureHistory.length>maxTemperatureSamples) temperatureHistory.shift();
...
function drawTemperatureChart(){
  ...
  let min=Math.min(...temperatureHistory);
  let max=Math.max(...temperatureHistory);
  if(max-min<2){min-=1;max+=1;}
  ctx.strokeStyle='#31d18b';
  ctx.lineWidth=3;
  ctx.beginPath();
  temperatureHistory.forEach((value,index)=>{
    const x=index*(width/(maxTemperatureSamples-1));
...
float readTemperatureC() {
  ...
  for (int i = 0; i < samples; i++) {
    sumMilliVolts += analogReadMilliVolts(TMP_PIN);
    delay(2);
  }
  const float averageMilliVolts = sumMilliVolts / static_cast<float>(samples);
  return (averageMilliVolts - 500.0f) / 10.0f;
}

void updateTemperature() {
  if (millis() - lastTemperatureRead < TEMPERATURE_INTERVAL_MS) return;
  lastTemperatureRead = millis();
  temperatureRaw = analogRead(TMP_PIN);
  temperatureMilliVolts = analogReadMilliVolts(TMP_PIN);
...
void initImu() {
  ...
  digitalWrite(BMI_SDO_PIN, HIGH); // HIGH = dirección 0x69

  Wire.begin(I2C_SDA, I2C_SCL);

  imuReady = (imu.beginI2C(BMI_ADDRESS, Wire) == BMI2_OK);
  Serial.println(imuReady
                 ? "BMI270 conectado en 0x69."
                 : "Error: BMI270 no detectado en 0x69.");
}
...
void handleState() {
  ...
  json += ",\"tempC\":" + String(temperatureC, 2);
  json += ",\"tempRaw\":" + String(temperatureRaw);
  json += ",\"tempMv\":" + String(temperatureMilliVolts);
